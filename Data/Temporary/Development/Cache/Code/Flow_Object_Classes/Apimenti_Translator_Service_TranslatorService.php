...
class TranslatorService_Original {
   ...
   private function runPullers() {
    ...
    public function translate($URL) {
        ...
        print $this->songHTML;
        $this->runPullers();
        print '<br>' . $this->songTitle;
        print '<br><pre>' . $this->songLyric . '</pre>';
        var_dump($this->artistName);
        var_dump($this->songChords);
 ...
 * 
 */
class TranslatorService extends TranslatorService_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	$reflectedClass = new \ReflectionClass('Apimenti\Translator\Service\TranslatorService');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if ($reflectionService->isPropertyTaggedWith('Apimenti\Translator\Service\TranslatorService', $propertyName, 'transient')) continue;
